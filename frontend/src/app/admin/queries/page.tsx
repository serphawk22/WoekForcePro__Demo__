...
const QueryAnalytics = dynamic(
  () =>
    import("@/components/admin/queries/QueryAnalytics").then((m) => m.QueryAnalytics),
  {
    ssr: false,
    loading: () => (
...
export default function AdminQueriesPage() {
  ...
  return (
    <ProtectedRoute allowedRoles={["admin"]}>
      <DashboardLayout role="admin" userName={user?.name} userHandle={user?.email ? `@${user.email.split("@")[0]}` : undefined}>
