...
export default function HappySheetPage() {
  ...
  const buildReportCanvas = (date: string, rows: Array<any>) => {
    ...
    const reportRows = (rows ?? []).map((row) => ({
      ...
      whatMadeYouHappy: String(row?.what_made_you_happy || "-"),
      whatMadeOthersHappy: String(row?.what_made_others_happy || "-"),
      goalsWithoutGreed: String(row?.goals_without_greed || "-"),
      dreamsSupported: String(row?.dreams_supported || "-"),
      goalsWithoutGreedImpossible: String(row?.goals_without_greed_impossible || "-"),
    }));

    const computedRows = reportRows.map((row) => {
      const cells = [
        row.userName,
        row.whatMadeYouHappy,
        row.whatMadeOthersHappy,
        row.goalsWithoutGreed,
        row.dreamsSupported,
        row.goalsWithoutGreedImpossible,
      ];
    ...
    ctx.fillRect(padding, tableTop, totalTableWidth, headerHeight);
    ctx.strokeStyle = "#e2e8f0";
    ctx.lineWidth = 1;
    ctx.strokeRect(padding, tableTop, totalTableWidth, headerHeight);
    ...
    computedRows.forEach((rowData, rowIdx) => {
      let cellX = padding;
      ctx.strokeStyle = "#e2e8f0";
      ctx.strokeRect(padding, y, totalTableWidth, rowData.height);
  ...
  const handleDownloadPng = async () => {
    ...
    try {
      ...
      downloadCanvasAsJpeg(canvas, `workforce-pro-happy-sheet-${reportDate}.jpg`);
    } catch (err: any) {
