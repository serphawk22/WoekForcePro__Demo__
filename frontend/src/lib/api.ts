 ...
 * Also writes a JS-readable cookie named "access_token" so the Next.js middleware
 * (which cannot access localStorage) can authenticate all protected routes.
 */
export function setAuth(token: string, user: Omit<LoginResponse, 'access_token' | 'token_type'>) {
  if (typeof window !== 'undefined') {
    ...
    // ─── CRITICAL FIX ────────────────────────────────────────────────────────
    // Next.js middleware runs on the Edge and can only read *cookies* — it has
    // no access to localStorage.  Without this cookie every navigation to an
    // /admin/* route triggers the middleware's "no token → redirect to /login"
    // branch, even though the user is legitimately authenticated.
    // SameSite=Lax; Path=/ ensures the cookie is sent on every same-origin
    // navigation. Expiry matches ACCESS_TOKEN_EXPIRE_MINUTES=480 (8 hours).
...
/**
 * Clear authentication data (localStorage + the middleware-readable cookie).
 */
export function clearAuth() {
  if (typeof window !== 'undefined') {
    ...
    // Expire the cookie so the middleware stops treating this session as authenticated.
...
export async function uploadTaskVoiceNote(
  ...
  for (let i = 0; i < uploadUrls.length; i += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          // Do NOT clearAuth() here — let the auth-401 listener in AuthProvider
          // decide whether to clear (it checks if the local token is still valid).
          if (typeof window !== 'undefined') {
...
export async function uploadProfilePicture(
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 401) {
        console.error('[API] 401 Unauthorized on upload-picture');
        // Do NOT clearAuth() here — let the auth-401 listener in AuthProvider
        // decide whether to clear (it checks if the local token is still valid).
        if (typeof window !== 'undefined') {
...
export interface SearchResult {
  ...
  subtitle: string;
  icon: string | null;
  type: "Employee" | "Project" | "Task" | "Ticket";
}
