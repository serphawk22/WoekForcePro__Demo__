    ...
    if not is_sqlite:
        ...
        with engine.connect() as conn:
            try:
                ...
                """))
                # Add compatibility values for legacy rows and older deployments.
                for enum_value in [
                    "INFO",
                    "SYSTEM",
                    "USER_APPROVED",
                ...
                for enum_value in [
                    ...
                    try:
                        conn.execute(text(f"ALTER TYPE userrole ADD VALUE IF NOT EXISTS '{enum_value}'"))
                    except Exception:
                        pass

                # Ensure old rows are normalized to valid enum values.
                try:
                    conn.execute(text("UPDATE users SET role = 'employee' WHERE role::text IN ('Supplier', 'supplier')"))
                except Exception:
                    pass
                try:
                    conn.execute(text("UPDATE notifications SET type = 'TASK_COMMENT' WHERE type::text = 'info'"))
                except Exception:
                    pass
                try:
                    conn.execute(text("UPDATE notifications SET type = 'TASK_COMMENT' WHERE type::text = 'system' OR type::text = 'SYSTEM'"))
                except Exception:
                    pass
